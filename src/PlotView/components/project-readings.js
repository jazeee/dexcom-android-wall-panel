const calculateSlopeInValuePerSec = (datum2, datum1) => {
  const {
    value: value1,
    timeSinceLastReadingInSeconds: timeInSec1,
  } = datum1;
  const {
    value: value2,
    timeSinceLastReadingInSeconds: timeInSec2,
  } = datum2;
...
export const projectReadings = (readingData) => {
  ...
  if (readingData.length >= 3) {
    ...
    for (let i = 1; i < 10; i += 1) {
      ...
      const timeSinceLastReadingInSeconds = latestTimeInSeconds + deltaTimeInSec;
